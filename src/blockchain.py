...
class Blockchain:
    def __init__(self, override_constants: Dict = {}):
        ...
        self.tips: List[SmallHeaderBlock] = []
        self.lca_block: SmallHeaderBlock
        ...
        self.headers: Dict[bytes32, SmallHeaderBlock] = {}

    async def initialize(self, small_header_blocks: Dict[str, SmallHeaderBlock]):
        ...
        if len(small_header_blocks) > 0:
            self.headers = small_header_blocks
            for _, small_header_block in small_header_blocks.items():
                self.height_to_hash[
                    small_header_block.height
                ] = small_header_block.header_hash
            for _, small_header_block in small_header_blocks.items():
                await self._reconsider_heads(small_header_block, False)
            assert (
                self.headers[self.height_to_hash[uint32(0)]]
                == self.genesis.header_block
            )
        if len(small_header_blocks) > 1:
            assert (
    ...
    async def receive_block(
        self, block: FullBlock, prev_block: Optional[HeaderBlock] = None
    ) -> ReceiveBlockResult:
        """
        ...
        if not await self.validate_block(block, prev_block, genesis):
    ...
    async def validate_block(
        ...
        if not genesis:
            ...
            ips = self.get_next_ips(prev_block)
        else:
