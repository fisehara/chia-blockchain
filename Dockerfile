...
RUN apt-get update
RUN apt-get install --assume-yes build-essential git cmake libssl-dev libffi-dev libgmp3-dev python3-virtualenv libboost-dev libboost-system-dev --no-install-recommends
...
RUN . .venv/bin/activate && make -C lib/chiavdf/fast_vdf